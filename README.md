...
eip_adapter [options]
  ...
  --out-inst N        出力 (O→T) アセンブリインスタンス番号 (既定 150)
  --in-inst  N        入力 (T→O) アセンブリインスタンス番号 (既定 100)
  --quiet             ログを抑制
  --help              ヘルプ
```
...
`src/device.h` で変更できます。I/O サイズは Forward Open の
Network Connection Parameters から動的に決まります。
